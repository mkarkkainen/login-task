...
const UITextInput = ({version, placeholder, icon }) => {
    ...
    return (
        <div className={
            `${classes.root} ${version === "default" ?
            classes.default : version === "focused" ?
            classes.focused : version === "disabled" ?
            classes.disabled : classes.variant }`
        }>
            {/* <img className={classes.icon} src={IconPerson} alt="" /> */}
        <div className={classes.icon}>
            {handleIcon(icon)}
        </div>
            <input
                onFocus={(e) => e.target.placeholder = ""}
                onBlur={(e) => e.target.placeholder ? e.target.placeholder = placeholder : null}
